...
object FrameStepFactory
  ...
  def fromXml(xml: Node): FrameStep = xml.label match {
    ...
    case DSAInput.tag          => DSAInput.fromXml(xml)
    case DSAOutput.tag         => DSAOutput.fromXml(xml)
  ...
  def fromJson(json: JValue): FrameStep = (json \ "tag" asString) match {
    ...
    case DSAInput.tag          => DSAInput.fromJson(json)
    case DSAOutput.tag         => DSAOutput.fromJson(json)
