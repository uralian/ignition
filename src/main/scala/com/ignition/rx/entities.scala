 ...
 *
 * @param target the block whose output is exposed as the pipeline's output.
 * @param starters the blocks that need to be reset each time the pipeline's `reset` method is called.
 */
