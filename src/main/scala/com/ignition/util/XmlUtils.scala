...
object XmlUtils {
  ...
  implicit class RichNodeSeq(val xml: NodeSeq) extends AnyVal {
    ...
    def asLong: Long = xml.head
    def getAsLong: Option[Long] = nodeSeqToOpt(xml)
    ...
    implicit private def nodeToLong(xml: Node): Long = xml.text.toLong
  ...
  implicit def longToText(n: Long) = Text(n.toString)
