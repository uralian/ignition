 ...
 */
sealed trait RowCondition extends Function1[Row, Boolean] with Serializable {
...
object RowConditions {
  ...
  case class AndCondition(c1: RowCondition, c2: RowCondition) extends RowCondition {
    def apply(row: Row) = c1(row) && c2(row)
...
case class FieldLiteral(name: String) {
  ...
  case class NeCondition(x: Any) extends RowCondition {
    def apply(row: Row) = x match {
      case f: FieldLiteral => eval(row) != f.eval(row)
      case a @ _ => eval(row) != a
    }
  ...
  def in(x: Any*) = new RowCondition {
    def apply(row: Row) = x contains (eval(row))
  }

  def matches(x: String) = new RowCondition {
    def apply(row: Row) = evalAs[String](row) matches x
  }
  def ~ = matches _
