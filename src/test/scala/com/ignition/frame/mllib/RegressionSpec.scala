...
class RegressionSpec extends FrameFlowSpecification {
  ...
  val grid = DataGrid(schema) rows (
    ...
    ("green", 35, 76, 120), ("blue", 10, 15, 75), ("green", 10, 26, 99),
    ("red", 45, 64, 105), ("red", 15, 25, 82), ("blue", 15, 34, 77))
  ...
  "Regression" should {
    "compute without grouping" in {
      ...
      assertSchema(double("y_weight") ~ double("z_weight") ~ double("intercept") ~ double("r2"), reg, 0)
    }
    "compute with grouping" in {
      val reg = Regression("x") features ("y", "z") groupBy "color" iterations 200 step 5
      grid --> reg

      assertSchema(string("color") ~ double("y_weight") ~ double("z_weight") ~ double("intercept") ~ double("r2"), reg, 0)
    }
